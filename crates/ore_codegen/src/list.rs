    ...
    pub(crate) fn compile_list_method(
        ...
        match method {
            ...
            "map" | "filter" | "flat_map" | "take_while" | "drop_while" => {
                ...
                let (lambda_fn, env_ptr) = self.resolve_lambda_arg(&args[0], &kinds, method, true)?;
            ...
            "partition" => {
                ...
                let elem_kind = self.last_list_elem_kind.clone();
                let kinds = vec![elem_kind.clone().unwrap_or(ValKind::Int)];
                let (lambda_fn, env_ptr) = self.resolve_lambda_arg(&args[0], &kinds, method, false)?;
                let fn_ptr = lambda_fn.as_global_value().as_pointer_value();
            ...
            "fold" => {
                ...
                let (lambda_fn, env_ptr) = self.resolve_lambda_arg(&args[1], &kinds, method, false)?;
                let fn_ptr = lambda_fn.as_global_value().as_pointer_value();
            ...
            "par_map" => {
                ...
                let (lambda_fn, env_ptr) = self.resolve_lambda_arg(&args[0], &[ValKind::Int], method, false)?;
                let fn_ptr = lambda_fn.as_global_value().as_pointer_value();
            ...
            "reduce" => {
                ...
                let kinds = vec![ValKind::Int, elem_kind.unwrap_or(ValKind::Int)];
                let (lambda_fn, env_ptr) = self.resolve_lambda_arg(fn_arg, &kinds, method, false)?;
                let fn_ptr = lambda_fn.as_global_value().as_pointer_value();
                if args.len() == 1 {
