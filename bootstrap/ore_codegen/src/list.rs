...
impl<'ctx> CodeGen<'ctx> {
    pub(crate) fn compile_list_method(
        ...
        match method {
            "len" | "is_empty" | "clear" | "reverse" | "flatten" | "dedup" | "enumerate"
            | "first" | "last" | "frequencies" => {
                self.list_method_simple(list_val, method, elem_kind)
            }
            "push" | "pop" | "get" | "set" | "get_or" | "insert" | "remove_at" => {
                self.list_method_access(list_val, method, args, func, elem_kind)
            }
            "map" | "filter" | "flat_map" | "take_while" | "drop_while" | "each" | "tap"
            | "map_with_index" | "each_with_index" | "par_map" | "par_each" | "any" | "all"
            | "find" | "find_index" | "partition" | "unique_by" | "to_map" => {
                self.list_method_lambda(list_val, method, args, func, elem_kind)
            }
            "fold" | "reduce" | "scan" => {
                self.list_method_fold(list_val, method, args, func, elem_kind)
            }
            "sort" | "sort_by" | "min_by" | "max_by" => {
                self.list_method_sort(list_val, method, args, func, elem_kind)
            }
            "sum" | "product" | "average" | "min" | "max" | "count" | "count_by" | "group_by" => {
                self.list_method_aggregate(list_val, method, args, func, elem_kind)
            }
            "join" | "contains" | "index_of" | "unique" | "zip" | "zip_with" | "slice" | "take"
            | "skip" | "step" | "window" | "chunks" | "intersperse" => {
                self.list_method_transform(list_val, method, args, func, elem_kind)
            }
            _ => Err(Self::unknown_method_error(
                "List",
                method,
                &[
                    "len",
    ...
    fn list_method_simple(
        ...
        match method {
            ...
            "is_empty" => {
                let len_val = self
                    .call_rt("ore_list_len", &[list_val.into()], "len")?
                    .into_int_value();
            ...
            "reverse" => {
                let rev_val =
                    self.call_rt("ore_list_reverse_new", &[list_val.into()], "reversed")?;
            ...
            "frequencies" => {
                ...
                let val = self.call_rt(
                    "ore_list_frequencies",
                    &[list_val.into(), kind_val.into()],
                    "freq",
                )?;
    ...
    fn list_method_access(
        ...
        match method {
            ...
            "get" => {
                ...
                let raw_val =
                    self.call_rt("ore_list_get", &[list_val.into(), idx.into()], "get")?;
                let typed_val = self.list_elem_from_i64(raw_val, elem_kind)?;
                Ok((typed_val, elem_kind.clone()))
            }
            "set" => {
                ...
                self.call_rt(
                    "ore_list_set",
                    &[list_val.into(), idx.into(), val_i64.into()],
                    "",
                )?;
                Ok((list_val, ValKind::list_of(elem_kind.clone())))
            }
            "get_or" => {
                ...
                let raw_val = self.call_rt(
                    "ore_list_get_or",
                    &[list_val.into(), idx.into(), default_i64.into()],
                    "getor",
                )?;
                let typed_val = self.list_elem_from_i64(raw_val, elem_kind)?;
                Ok((typed_val, elem_kind.clone()))
            ...
            "insert" => {
                ...
                self.call_rt(
                    "ore_list_insert",
                    &[list_val.into(), idx.into(), i64_val.into()],
                    "",
                )?;
                Ok((list_val, ValKind::list_of(elem_kind.clone())))
            }
    ...
    fn list_method_lambda(
        ...
        match method {
            "map" | "filter" | "flat_map" | "take_while" | "drop_while" => {
                ...
                let (fn_ptr, env_ptr, ret_kind) =
                    self.resolve_lambda_arg(&args[0], &kinds, method)?;
            ...
            "map_with_index" => {
                ...
                let val = self.call_rt(
                    "ore_list_map_with_index",
                    &[list_val.into(), fn_ptr.into(), env_ptr.into()],
                    "mwi",
                )?;
                Ok((val, ValKind::list_of(ret_kind)))
            }
            "each_with_index" => {
            ...
            "par_map" => {
                ...
                let (fn_ptr, env_ptr, ret_kind) =
                    self.resolve_lambda_arg(&args[0], &[ValKind::Int], method)?;
            ...
            "any" | "all" => {
                ...
                let bool_val = bld!(self.builder.build_int_truncate(
                    val.into_int_value(),
    ...
    fn list_method_aggregate(
        ...
        match method {
            ...
            "average" => {
                let rt_name = if matches!(elem_kind, ValKind::Float) {
                    "ore_list_average_float"
                } else {
                    "ore_list_average"
                };
            ...
            "count_by" | "group_by" => {
                ...
                let val_kind = if method == "count_by" {
                    ValKind::Int
                } else {
                    ValKind::list_of(elem_kind.clone())
                };
        ...
        match method {
            ...
            "contains" => {
                ...
                let i8_val = if matches!(elem_kind, ValKind::Str) {
                    self.call_rt(
                        "ore_list_contains_str",
                        &[list_val.into(), val.into()],
                        "lcontains",
                    )?
                } else {
                    let i64_val = self.value_to_i64(val)?;
            ...
            "index_of" => {
                ...
                let rt_name = if matches!(elem_kind, ValKind::Str) {
                    "ore_list_index_of_str"
                } else {
                    "ore_list_index_of"
                };
            ...
            "zip_with" => {
                ...
                let kinds = [ek.clone(), ek];
                let (fn_ptr, env_ptr, ret_kind) =
                    self.resolve_lambda_arg(&args[1], &kinds, method)?;

                let val = self.call_rt(
                    "ore_list_zip_with",
                    &[list_val.into(), other.into(), fn_ptr.into(), env_ptr.into()],
                    "zipw",
                )?;
                Ok((val, ValKind::list_of(ret_kind)))
            }
            "slice" => {
                ...
                let val = self.call_rt(
                    "ore_list_slice",
                    &[list_val.into(), start.into(), end.into()],
                    "lslice",
                )?;
                Ok((val, ValKind::list_of(elem_kind.clone())))
            }
            ...
            "intersperse" => {
                ...
                let val = self.call_rt(
                    "ore_list_intersperse",
                    &[list_val.into(), sep_i64.into()],
                    "inter",
                )?;
                Ok((val, ValKind::list_of(elem_kind.clone())))
            }
    ...
    pub(crate) fn compile_list_lit(
        ...
        for elem in elements {
            ...
            bld!(self
                .builder
                .build_call(list_push, &[list_ptr.into(), push_val.into()], ""))?;
        }
    ...
    pub(crate) fn compile_list_comp(
        ...
        // Create output list
        let list_ptr = self
            .call_rt("ore_list_new", &[], "comp_list")?
            .into_pointer_value();
        ...
        if is_range {
            ...
            let cmp =
                bld!(self
                    .builder
                    .build_int_compare(IntPredicate::SLT, cur, end_val, "comp_cmp"))?;
            ...
            if let Some(c) = cond {
                ...
                bld!(self
                    .builder
                    .build_conditional_branch(bool_val, filter_bb, inc_bb))?;
            ...
            let next = bld!(self
                .builder
                .build_int_add(cur, i64_type.const_int(1, false), "inc"))?;
            ...
            let len_val = self
                .call_rt("ore_list_len", &[list_src.into()], "len")?
                .into_int_value();
            ...
            let cmp =
                bld!(self
                    .builder
                    .build_int_compare(IntPredicate::SLT, idx, len_val, "comp_cmp"))?;
            bld!(self.builder.build_conditional_branch(cmp, body_bb, end_bb))?;
